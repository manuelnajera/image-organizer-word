...
Sub OrganizeImagesAndDelete()
    ...
    layout = InputBox("Images per page (1, 2, 4, 6, 8, 10, up to 20):")
    If layout < 1 Or layout > 20 Then
        ' Show an alert for invalid value
        MsgBox "Invalid value! Please enter a value between 1 and 20.", vbExclamation
        Exit Sub ' Exit the subroutine
    End If
    
    If layout > 2 Then
    imagesPerLine = InputBox("Images per row (1, 2, 3, 4, 5):")
    Else
    imagesPerLine = 1
    End If
    ...
    If imagesPerLine < 1 Or imagesPerLine > 5 Then
        ' Show an alert for invalid value
        MsgBox "Invalid value! Please enter a value between 1 and 5.", vbExclamation
        Exit Sub ' Exit the subroutine
    End If
...
If wdRng.InlineShapes.Count > 0 Then
    ...
    wdRng.InsertBefore "  " ' Insert a space
                ...
                Else
                    MsgBox "No images found in the document."
                End If
    ...
    ' Save the document
    wdDoc.SaveAs2 fileName:=ThisDocument.Path & "/OrganizedImages.docx"
